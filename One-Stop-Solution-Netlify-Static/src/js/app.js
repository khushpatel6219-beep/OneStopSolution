...
class AppController {
  ...
  openSendSmsModal(clientName, policyDesc, dueDate, amount) {
        ...
        <div class="w-full max-w-lg zoho-card p-6 rounded-2xl border border-amber-500/40 shadow-2xl space-y-4">
          ...
          <form id="form-dispatch-sms-text" class="space-y-4 text-xs">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <button type="submit" id="btn-dispatch-native-sms" class="py-3 px-4 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-extrabold transition flex items-center justify-center gap-2 shadow-lg text-center text-xs">
                <i data-lucide="message-square" class="w-4 h-4"></i> Send Text Message
              </button>
              <button type="button" id="btn-dispatch-whatsapp-direct" class="py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold transition flex items-center justify-center gap-2 text-center text-xs">
                <i data-lucide="smartphone" class="w-4 h-4"></i> Send WhatsApp Message
              </button>
            </div>
          </form>
    ...
    document.getElementById('btn-dispatch-whatsapp-direct').onclick = () => {
      ...
      this.closeGlobalModal();
      this.showToast(`💬 Opening WhatsApp to send message to ${phoneInput}!`);
    };

    // 2. Text Message Submit Handler (Direct MSG91 Dispatch)
    document.getElementById('form-dispatch-sms-text').onsubmit = async (e) => {
      ...
      this.showToast(`📡 Dispatching Text Message directly via MSG91 to ${phoneInput}...`);

      try {
        const result = await store.sendMsg91Sms(cleanPhone, msg);

        store.addReminderLog({
          clientName: client.name,
          invoiceNumber: `POL-${Date.now().toString().slice(-4)}`,
          reminderType: dueDate === '2026-07-28' ? 'Due Today Text Message' : 'Upcoming Expiry Text Message',
        ...
        this.closeGlobalModal();
        this.showToast(`✅ Text Message Sent Successfully via MSG91 to ${phoneInput}!`);
      } catch (err) {
        store.addReminderLog({
          clientName: client.name,
          invoiceNumber: `POL-${Date.now().toString().slice(-4)}`,
        ...
        this.closeGlobalModal();
        this.showToast(`✅ Message recorded & queued for ${phoneInput}!`);
      }
    };
  }
